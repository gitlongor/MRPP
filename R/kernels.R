skernel=eval(bquote(function(kernel= .kernels, eps=1e-3)
...
## Optimized based on the fact that ^2 and * are faster than other ^ powers
	...
	if(identical(environment(ans),environment())) environment(ans)=pkgEnv
	ans
},  # of function
constEnv) # of bquote
) # of eval
...
dkernel=eval(bquote(function(kernel= .kernels)
...
krRkernel=eval(bquote(function(kernel=.kernels)
...
ddkernel=eval(bquote(function(kernel=.kernels)
{
	kernel=match.arg(kernel)
	ans=switch(kernel,
...
d2dkernel=eval(bquote(function(kernel=.kernels)
{
	kernel=match.arg(kernel)
	ans=switch(kernel,
		gaussian=,normal=function(x){
...
ckernel=fourier.kernel=eval(bquote(function(kernel= .kernels, root.2pi=FALSE, const=NULL)
{
	kernel=match.arg(kernel)

	ans=switch(kernel,
	gaussian=,normal=function(s) exp(-.5*s*s) *..iroot2pi,
	dgaussian=,dnormal=function(s) exp(-.5*s*s)*s*(0-1i)*..iroot2pi,
	cosine=function(s)	.(pipi)*cos(s)/(.(pipi)-4*s*s) * ..iroot2pi,
	uniform = , rectangular = function(s){
		sinc(s) * ..iroot2pi
	},
	triangular = function(s){
		s2=s*s;
		ans=2*(1-cos(s))/s2 * ..iroot2pi
		idx=which(abs(s)<1e-2) ## close to 0/0 region: taylor series
		ans[idx]=(1 + s2[idx]*(s2[idx]*.(one360) - .(one12))) * ..iroot2pi
		ans
	},
	epanechnikov =, parabolic= function(s){
		s2=s*s;
		ans=3*(sin(s)-s*cos(s))/(s2*s) * ..iroot2pi
		idx=which(abs(s)<1e-2) ## close to 0/0 region: taylor series
		ans[idx]=(1 + s2[idx]*(s2[idx]*.(one280)-0.1)) * ..iroot2pi
		ans
	},
	biweight = , quartic=function(s){
		ss=sin(s); s2=s*s;
		ans=(45*(ss - s*cos(s)) - 15*s2*ss)/(s2*s2*s) * ..iroot2pi
		idx=which(abs(s)<5e-2) ## close to 0/0 region: taylor series
		ans[idx]=(1 + s2[idx]*(s2[idx]*(.(one504) - s2[idx]*.(one33264))-.(one14))) * ..iroot2pi
		ans
	},
	triweight = function(s){
		cs=cos(s); ss=sin(s);
		s2=s*s; #s4=s2*s2; s6=s4*s2;
		ans=105 * (s*(s2-15)*cs + (15-6*s2) *ss)  /(s2*s2*s2*s) *..iroot2pi
		#ans=(105*(s2*s*cs + 15*(ss -s*cs) - 6*s2*ss))/(s6*s) *..iroot2pi
		idx=which(abs(s)<1e-1) ## close to 0/0 region
		ans[idx]=(1 + s2[idx]*( s2[idx]*(.(one792) - s2[idx]*.(one61776)) -.(one18)))* ..iroot2pi
		ans
	},
	tricube = function(s){
		cs=cos(s); ss=sin(s);
		...
		ans=(.(two80d9)*(20160 - s6 - 9*(2240 - 1120*s2 + 80*s4 - s6)*cs -
			s*(20160 - 3240*s2 + 108*s4)*ss))/(s4*s6) *..iroot2pi
		idx=which(abs(s)<3.5e-1) ## close to 0/0 region
		ans[idx]=(1 + s2[idx]*(s2[idx]*(.(one528) - s2[idx]*(s2[idx]*.(one4199040)-.(one37440))) -.(three5d486))) *..iroot2pi
		ans
   },
	logistic =function(s){
		ps=.(base::pi)*s
		ans=ps/sinh(ps) *..iroot2pi
		idx=which(abs(s)<1e-2) ## close to 0/0 region
		ps2=ps*ps; ps4=ps2*ps2
		ans[idx]=(1 + ps2[idx]*(ps2[idx]*(.(seven360) - ps2[idx]*.(three1d15120)) -.(one6))) *..iroot2pi
		ans
	},
	sech = function(s){
		1/cosh(.(pid2) *s) *..iroot2pi
	}
	)  # of switch
	#))
	...
	if(root.2pi) {
		...
		{# code from bquote
			if (length(expr) >= 2L)
				as.call(lapply(expr, fix.iroot2pi))
			else if (length(expr) == 0L) expr
			else if (expr == as.name("..iroot2pi"))
				.(iroot.2pi)
			else expr
		}
	}else {
		...
		{# code from bquote
			if (length(expr) <= 1L) {
				expr
			}else if (expr[[1L]] == as.name('*') && expr[[3L]]== as.name("..iroot2pi")) {
				expr[[2L]]
			}else as.call(lapply(expr, fix.iroot2pi))
		}
	}
	body(ans) = fix.iroot2pi(body(ans))
...
mkernel=eval(bquote(function(kernel=.kernels, order=1:4)
{
	kernel=match.arg(kernel)
...
Rkernel=eval(bquote(function(kernel=.kernels, deriv.order=0:2)
{
	kernel=match.arg(kernel)
