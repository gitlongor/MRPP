...
nbinom.quadLL = function(y, size = 1/invsize, invsize=1/size,  standardize = FALSE)
{
	stopifnot(all(size==1/invsize || invsize==1/size))
	d=dim(y); 
...
nbinom.symm = function(y,size = 1/invsize, invsize=1/size)
{
	stopifnot(all(size==1/invsize || invsize==1/size))
	d=dim(y); 
	if(length(d)==2L && length(invsize)==d[2L]) invsize=rep(invsize,each=d[1L])
...
pois.quadLL = function(y, standardize=FALSE)
{
	ans = y^oneThird
	if(standardize) ans * y^oneSixth * 3 else ans
}
pois.symm = function(y, adjust=c('none','anscombe'))
{
	adjust=match.arg(adjust)
...
binom.quadLL = function(y, size, adjust=c('none','borges'), standardize=FALSE)
{
	adjust=match.arg(adjust)
	if(adjust=='borges'){
...
binom.symm   = function(y, size, adjust=c('none','borges'), standardize=FALSE)
{
	adjust=match.arg(adjust)
	d=dim(y)
	if(length(d)==2L && length(size)==d[2L]) size=rep(size, each=d[1L])
...
chisq.symm = function(y, df, ncp=0, adjust=c('abdel-aty','none'), standardize=FALSE)
{
	adjust = match.arg(adjust)
	if(adjust=='abdel-aty') {
		dfncp=df+ncp
...
fisher.z = function(y, df, adjust=c('none','hotelling'))
{
	ans = atanh(y)
	adjust=match.arg(adjust)
	if(adjust=='hotelling') (ans-y/2/(df-1.5))*sqrt(df-fourThirds) else ans 
}
environment(fisher.z) = constEnv
